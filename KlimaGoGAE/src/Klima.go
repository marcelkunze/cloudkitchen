...
func init() {
        ...
        http.HandleFunc("/gegenwart",handleGegenwart)
        http.HandleFunc("/komet",handleKomet)
        http.HandleFunc("/treibhaus", handleTreibhaus)
}

func root(w http.ResponseWriter, r *http.Request) {
    fmt.Fprint(w, KlimaForm)
}

const KlimaForm = `
<html>
  <head>
    <title>Klimasimulation</title>
  </head>
  <body>
  <H1>Programmierung in Go!</h1>
    <br/>
    Waehlen Sie das Szenario<br/>
    <ul>
      <li><a href="gegenwart">Gegenwart</a></li>
      <li><a href="komet">Komet</a></li>
      <li><a href="treibhaus">Treibhaus</a></li>
    </ul>
  </body>
</html> `

func handleGegenwart(w http.ResponseWriter, r *http.Request) {
	fmt.Fprint(w, "Gegenwart ", Simulation(280.,320.,0.30,0.11,0.31,0.53,0.06))
}

func handleKomet(w http.ResponseWriter, r *http.Request) {
	fmt.Fprint(w, "Komet     ", Simulation(280.,320.,0.36,0.11,0.37,0.43,0.05)) 
}

func handleTreibhaus(w http.ResponseWriter, r *http.Request) {
	fmt.Fprint(w, "Treibhaus ", Simulation(280.,320.,0.30,0.10,0.34,0.53,0.05))
}

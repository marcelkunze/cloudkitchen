/**	4.Aufgabenblatt: Iterierte Funktionssysteme (IFS)
	Implementation der Klasse IFS. IFS erhält durch den Konstruktor
	den IFS-Code (Parameter a-f). Die Wahrscheinlichkeiten der Affinitäten
	sind in Vektor p gespeichert. Zur Demonstration wurden die IFS-Codes für
	Farn, Baum, Sierpinski-Dreieck und Kochsche Kurve programmiert.
	Das Programm wird durch Angabe zweier Parameter für den IFS-Code und die
	Zahl der Iterationen angesteuert.
	IFS ist lauffähig als Applikation und als Applet.
*/
...
public class IFS extends Applet
{
	...
	{
		standAlone = true;				// Laeuft als Applikation

		// Auswerten der Parameterliste
		if (args.length != 0) IFS.selection = args[0];
		if (args.length > 1)  IFS.maxIter = Integer.parseInt(args[1]);
		System.out.println(IFS.selection+":"+IFS.maxIter+" Iterationen");

		Frame frame = new Frame("Iterierte Funktionssysteme");
		frame.show();
        	frame.setSize(800,480);
		IFS ifs = new IFS();
		frame.add("Center", ifs);
		ifs.init();
		ifs.start();
        	frame.show();
	}
